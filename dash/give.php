...
*Le captcha est déjà vérifié; On a $_SESSION['adresseDASH']
*
*Si on vient de l'accueil :
*On a $_POST['adresseDASH']
*
*Si on vient de faucet :
*on a $_SESSION['adresseDASH'] et $_GET['ajoutDASH']
*
*/
include 'Fonctions/connectionBDD.php';
$req = $bdd->query("SELECT `Gain2Min` FROM `CoursCryptos` WHERE `Code` = 'DASH'");
...
if (!empty($_POST['adresseDASH'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseDASH'] = htmlspecialchars($_POST['adresseDASH']);
		echo "session : " . $_SESSION['adresseDASH'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `DASH` WHERE `AdresseDASH` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseDASH'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `DASH` (`AdresseDASH`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseDASH'])));
			...
			$req = $bdd->prepare("UPDATE `DASH` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseDASH` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseDASH'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseDASH']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `DASH` SET `ActionsRestantes`= 0 WHERE `AdresseDASH` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseDASH'])));
			...
			$req = $bdd->prepare("UPDATE `DASH` SET `Gains` = `Gains`+ ? WHERE `AdresseDASH` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseDASH'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseDASH']) && empty($_GET['ajoutDASH']) && empty($_POST['adresseDASH']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `DASH` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseDASH` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseDASH'])));
			header("Location: faucet.php");
			exit();
		}
