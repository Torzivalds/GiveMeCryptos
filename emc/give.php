...
*Le captcha est déjà vérifié; On a $_SESSION['adresseEMC']
*
*Si on vient de l'accueil :
*On a $_POST['adresseEMC']
*
*Si on vient de faucet :
*on a $_SESSION['adresseEMC'] et $_GET['ajoutEMC']
*
*/
...
if (!empty($_POST['adresseEMC'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseEMC'] = htmlspecialchars($_POST['adresseEMC']);
		echo "session : " . $_SESSION['adresseEMC'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `EMC` WHERE `AdresseEMC` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseEMC'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `EMC` (`AdresseEMC`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseEMC'])));
			...
			$req = $bdd->prepare("UPDATE `EMC` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseEMC` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseEMC'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseEMC']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `EMC` SET `ActionsRestantes`= 0 WHERE `AdresseEMC` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseEMC'])));
			...
			$req = $bdd->prepare("UPDATE `EMC` SET `Gains` = `Gains`+ ? WHERE `AdresseEMC` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseEMC'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseEMC']) && empty($_GET['ajoutEMC']) && empty($_POST['adresseEMC']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `EMC` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseEMC` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseEMC'])));
			header("Location: faucet.php");
			exit();
		}
