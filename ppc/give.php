...
*Le captcha est déjà vérifié; On a $_SESSION['adressePPC']
*
*Si on vient de l'accueil :
*On a $_POST['adressePPC']
*
*Si on vient de faucet :
*on a $_SESSION['adressePPC'] et $_GET['ajoutPPC']
*
*/
include 'Fonctions/connectionBDD.php';
$req = $bdd->query("SELECT `Gain2Min` FROM `CoursCryptos` WHERE `Code` = 'PPC'");
...
if (!empty($_POST['adressePPC'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adressePPC'] = htmlspecialchars($_POST['adressePPC']);
		echo "session : " . $_SESSION['adressePPC'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `PPC` WHERE `AdressePPC` = ?");
		$req->execute(array(htmlspecialchars($_POST['adressePPC'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `PPC` (`AdressePPC`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adressePPC'])));
			...
			$req = $bdd->prepare("UPDATE `PPC` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdressePPC` = ?");
			$req->execute(array(htmlspecialchars($_POST['adressePPC'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adressePPC']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `PPC` SET `ActionsRestantes`= 0 WHERE `AdressePPC` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adressePPC'])));
			...
			$req = $bdd->prepare("UPDATE `PPC` SET `Gains` = `Gains`+ ? WHERE `AdressePPC` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adressePPC'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adressePPC']) && empty($_GET['ajoutPPC']) && empty($_POST['adressePPC']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `PPC` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdressePPC` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adressePPC'])));
			header("Location: faucet.php");
			exit();
		}
