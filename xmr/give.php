...
*Le captcha est déjà vérifié; On a $_SESSION['adresseXMR']
*
*Si on vient de l'accueil :
*On a $_POST['adresseXMR']
*
*Si on vient de faucet :
*on a $_SESSION['adresseXMR'] et $_GET['ajoutXMR']
*
*/
...
if (!empty($_POST['adresseXMR'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseXMR'] = htmlspecialchars($_POST['adresseXMR']);
		echo "session : " . $_SESSION['adresseXMR'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `XMR` WHERE `AdresseXMR` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseXMR'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `XMR` (`AdresseXMR`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseXMR'])));
			...
			$req = $bdd->prepare("UPDATE `XMR` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseXMR` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseXMR'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseXMR']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `XMR` SET `ActionsRestantes`= 0 WHERE `AdresseXMR` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseXMR'])));
			...
			$req = $bdd->prepare("UPDATE `XMR` SET `Gains` = `Gains`+ ? WHERE `AdresseXMR` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseXMR'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseXMR']) && empty($_GET['ajoutXMR']) && empty($_POST['adresseXMR']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `XMR` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseXMR` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseXMR'])));
			header("Location: faucet.php");
			exit();
		}
