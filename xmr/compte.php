...
$req = $bdd->query("SELECT `RetraitMin` FROM `CoursCryptos` WHERE `Code` = 'XMR'");
					...
					<div class="row">
						<div class="col-md-8 col-md-offset-2 text-center ubea-heading">
							...
							if (!empty($_SESSION['adresseXMR'])) {
								include 'Fonctions/connectionBDD.php';
								$req = $bdd->prepare("SELECT `Gains`, `DateCreation` FROM `XMR` WHERE `AdresseXMR` = ?");
								...
								?>
								<p>Vous avez <?php echo $Gains; ?> XMR sur votre compte.</p>
								<?php
								if ($Gains > $RetraitMin) {
									?>
									<a href="transfert.php">Demander le transfert des Moneros</a>
									<?php
								}
							}
							else{
