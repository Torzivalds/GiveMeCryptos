...
*Le captcha est déjà vérifié; On a $_SESSION['adresseRDD']
*
*Si on vient de l'accueil :
*On a $_POST['adresseRDD']
*
*Si on vient de faucet :
*on a $_SESSION['adresseRDD'] et $_GET['ajoutRDD']
*
*/
...
if (!empty($_POST['adresseRDD'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseRDD'] = htmlspecialchars($_POST['adresseRDD']);
		echo "session : " . $_SESSION['adresseRDD'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `RDD` WHERE `AdresseRDD` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseRDD'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `RDD` (`AdresseRDD`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseRDD'])));
			...
			$req = $bdd->prepare("UPDATE `RDD` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseRDD` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseRDD'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseRDD']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `RDD` SET `ActionsRestantes`= 0 WHERE `AdresseRDD` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseRDD'])));
			...
			$req = $bdd->prepare("UPDATE `RDD` SET `Gains` = `Gains`+ ? WHERE `AdresseRDD` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseRDD'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseRDD']) && empty($_GET['ajoutRDD']) && empty($_POST['adresseRDD']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `RDD` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseRDD` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseRDD'])));
			header("Location: faucet.php");
			exit();
		}
