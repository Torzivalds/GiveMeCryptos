...
*Le captcha est déjà vérifié; On a $_SESSION['adresseBCH']
*
*Si on vient de l'accueil :
*On a $_POST['adresseBCH']
*
*Si on vient de faucet :
*on a $_SESSION['adresseBCH'] et $_GET['ajoutBCH']
*
*/
...
if (!empty($_POST['adresseBCH'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseBCH'] = htmlspecialchars($_POST['adresseBCH']);
		echo "session : " . $_SESSION['adresseBCH'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `BCH` WHERE `AdresseBCH` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseBCH'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `BCH` (`AdresseBCH`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseBCH'])));
			...
			$req = $bdd->prepare("UPDATE `BCH` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseBCH` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseBCH'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseBCH']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `BCH` SET `ActionsRestantes`= 0 WHERE `AdresseBCH` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseBCH'])));
			...
			$req = $bdd->prepare("UPDATE `BCH` SET `Gains` = `Gains`+ ? WHERE `AdresseBCH` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseBCH'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseBCH']) && empty($_GET['ajoutBCH']) && empty($_POST['adresseBCH']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `BCH` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseBCH` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseBCH'])));
			header("Location: faucet.php");
			exit();
		}
