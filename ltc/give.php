...
*Le captcha est déjà vérifié; On a $_SESSION['adresseLTC']
*
*Si on vient de l'accueil :
*On a $_POST['adresseLTC']
*
*Si on vient de faucet :
*on a $_SESSION['adresseLTC'] et $_GET['ajoutLTC']
*
*/
include 'Fonctions/connectionBDD.php';
$req = $bdd->query("SELECT `Gain2Min` FROM `CoursCryptos` WHERE `Code` = 'LTC'");
...
if (!empty($_POST['adresseLTC'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseLTC'] = htmlspecialchars($_POST['adresseLTC']);
		echo "session : " . $_SESSION['adresseLTC'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `LTC` WHERE `AdresseLTC` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseLTC'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `LTC` (`AdresseLTC`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseLTC'])));
			...
			$req = $bdd->prepare("UPDATE `LTC` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseLTC` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseLTC'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseLTC']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `LTC` SET `ActionsRestantes`= 0 WHERE `AdresseLTC` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseLTC'])));
			...
			$req = $bdd->prepare("UPDATE `LTC` SET `Gains` = `Gains`+ ? WHERE `AdresseLTC` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseLTC'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseLTC']) && empty($_GET['ajoutLTC']) && empty($_POST['adresseLTC']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `LTC` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseLTC` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseLTC'])));
			header("Location: faucet.php");
			exit();
		}
