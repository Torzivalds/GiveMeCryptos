...
*Le captcha est déjà vérifié; On a $_SESSION['adresseXPM']
*
*Si on vient de l'accueil :
*On a $_POST['adresseXPM']
*
*Si on vient de faucet :
*on a $_SESSION['adresseXPM'] et $_GET['ajoutXPM']
*
*/
...
if (!empty($_POST['adresseXPM'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseXPM'] = htmlspecialchars($_POST['adresseXPM']);
		echo "session : " . $_SESSION['adresseXPM'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `XPM` WHERE `AdresseXPM` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseXPM'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `XPM` (`AdresseXPM`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseXPM'])));
			...
			$req = $bdd->prepare("UPDATE `XPM` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseXPM` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseXPM'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseXPM']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `XPM` SET `ActionsRestantes`= 0 WHERE `AdresseXPM` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseXPM'])));
			...
			$req = $bdd->prepare("UPDATE `XPM` SET `Gains` = `Gains`+ ? WHERE `AdresseXPM` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseXPM'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseXPM']) && empty($_GET['ajoutXPM']) && empty($_POST['adresseXPM']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `XPM` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseXPM` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseXPM'])));
			header("Location: faucet.php");
			exit();
		}
