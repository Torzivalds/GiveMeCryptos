...
*Le captcha est déjà vérifié; On a $_SESSION['adresseBNC']
*
*Si on vient de l'accueil :
*On a $_POST['adresseBNC']
*
*Si on vient de faucet :
*on a $_SESSION['adresseBNC'] et $_GET['ajoutBNC']
*
*/
...
if (!empty($_POST['adresseBNC'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseBNC'] = htmlspecialchars($_POST['adresseBNC']);
		echo "session : " . $_SESSION['adresseBNC'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `BNC` WHERE `AdresseBNC` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseBNC'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `BNC` (`AdresseBNC`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseBNC'])));
			...
			$req = $bdd->prepare("UPDATE `BNC` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseBNC` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseBNC'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseBNC']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `BNC` SET `ActionsRestantes`= 0 WHERE `AdresseBNC` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseBNC'])));
			...
			$req = $bdd->prepare("UPDATE `BNC` SET `Gains` = `Gains`+ ? WHERE `AdresseBNC` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseBNC'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseBNC']) && empty($_GET['ajoutBNC']) && empty($_POST['adresseBNC']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `BNC` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseBNC` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseBNC'])));
			header("Location: faucet.php");
			exit();
		}
