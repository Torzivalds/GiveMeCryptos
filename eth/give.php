...
*Le captcha est déjà vérifié; On a $_SESSION['adresseETH']
*
*Si on vient de l'accueil :
*On a $_POST['adresseETH']
*
*Si on vient de faucet :
*on a $_SESSION['adresseETH'] et $_GET['ajoutETH']
*
*/
...
if (!empty($_POST['adresseETH'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseETH'] = htmlspecialchars($_POST['adresseETH']);
		echo "session : " . $_SESSION['adresseETH'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `ETH` WHERE `AdresseETH` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseETH'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `ETH` (`AdresseETH`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseETH'])));
			...
			$req = $bdd->prepare("UPDATE `ETH` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseETH` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseETH'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseETH']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `ETH` SET `ActionsRestantes`= 0 WHERE `AdresseETH` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseETH'])));
			...
			$req = $bdd->prepare("UPDATE `ETH` SET `Gains` = `Gains`+ ? WHERE `AdresseETH` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseETH'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseETH']) && empty($_GET['ajoutETH']) && empty($_POST['adresseETH']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `ETH` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseETH` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseETH'])));
			header("Location: faucet.php");
			exit();
		}
