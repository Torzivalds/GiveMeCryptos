...
$req = $bdd->query("SELECT `RetraitMin` FROM `CoursCryptos` WHERE `Code` = 'ETH'");
					...
					<div class="row">
						<div class="col-md-8 col-md-offset-2 text-center ubea-heading">
							...
							if (!empty($_SESSION['adresseETH'])) {
								include 'Fonctions/connectionBDD.php';
								$req = $bdd->prepare("SELECT `Gains`, `DateCreation` FROM `ETH` WHERE `AdresseETH` = ?");
								...
								?>
								<p>Vous avez <?php echo $Gains; ?> ETH sur votre compte.</p>
								<?php
								if ($Gains > $RetraitMin) {
									?>
									<a href="transfert.php">Demander le transfert des Ethereums</a>
									<?php
								}
							}
							else{
						...
						<div class="row">
							...
							<div class="col-md-12">
								<div class="feature-left">
									...
									<div class="feature-copy">
										<h3>Transfert des Ethereums</h3>
										<p>Les Ethereums de votre compte Give Me Ethereum ne peuvent être transfèrés que vers l'adresse publique avec laquelle vous vous connectez. Pour qu'un transfert puisse avoir lieu, le compte Give Me Ethereum doit avoir été créé au moins 30 jours avant. Le montant minimum est de 0.05 ETH. Remarque : Les transferts ne sont pas automatiques; le transfert peut donc avoir lieu plusieurs jours après l'avoir demandé. La demade se fait sur cette page, lorsque vous avez un compte d'au moins 30 jours qui a au minimum <?php echo $RetraitMin; ?> ETH.</p>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							<div class="col-md-12">
								<div class="feature-left">
									...
									<div class="feature-copy">
										<h3>Sécurité</h3>
										<p>Give Me Ethereum n'utilise même pas de mot de passe, mais il est tout de même très sécurisé : Le site demande un captcha pour se connecter, ce qui rends plus compliqué aux robots de se connecter. Ensuite, les fonds ne peuvent être envoyés QUE dans le porte monnaie Ethereum qui correspond à l'adresse du compte, ce qui fait qu'un pirate ne pourra pas se transfèrer les fonds dans son porte monnaie Ethereum.</p>
									</div>
								</div>
							</div>
							<div class="col-md-12">
							...
							<div class="col-md-12">
								<div class="feature-left">
									...
									<div class="feature-copy">
										<h3>Contact</h3>
										<p>Pour toute question à propos du site de Give Me Ethereum, pour tous problèmes survenus sur ce site ou tout autre chose, veuillez nous contacter via le formulaire de contact sur cette page : <a href="contact.php">Contactez-nous ici</a></p>
									</div>
								</div>
							</div>
						</div>
