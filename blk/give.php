...
*Le captcha est déjà vérifié; On a $_SESSION['adresseBLK']
*
*Si on vient de l'accueil :
*On a $_POST['adresseBLK']
*
*Si on vient de faucet :
*on a $_SESSION['adresseBLK'] et $_GET['ajoutBLK']
*
*/
...
if (!empty($_POST['adresseBLK'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseBLK'] = htmlspecialchars($_POST['adresseBLK']);
		echo "session : " . $_SESSION['adresseBLK'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `BLK` WHERE `AdresseBLK` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseBLK'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `BLK` (`AdresseBLK`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseBLK'])));
			...
			$req = $bdd->prepare("UPDATE `BLK` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseBLK` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseBLK'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseBLK']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `BLK` SET `ActionsRestantes`= 0 WHERE `AdresseBLK` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseBLK'])));
			...
			$req = $bdd->prepare("UPDATE `BLK` SET `Gains` = `Gains`+ ? WHERE `AdresseBLK` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseBLK'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseBLK']) && empty($_GET['ajoutBLK']) && empty($_POST['adresseBLK']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `BLK` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseBLK` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseBLK'])));
			header("Location: faucet.php");
			exit();
		}
