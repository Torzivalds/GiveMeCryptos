...
*Le captcha est déjà vérifié; On a $_SESSION['adresseXRP']
*
*Si on vient de l'accueil :
*On a $_POST['adresseXRP']
*
*Si on vient de faucet :
*on a $_SESSION['adresseXRP'] et $_GET['ajoutXRP']
*
*/
include 'Fonctions/connectionBDD.php';
$req = $bdd->query("SELECT `Gain2Min` FROM `CoursCryptos` WHERE `Code` = 'XRP'");
...
if (!empty($_POST['adresseXRP'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseXRP'] = htmlspecialchars($_POST['adresseXRP']);
		echo "session : " . $_SESSION['adresseXRP'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `XRP` WHERE `AdresseXRP` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseXRP'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `XRP` (`AdresseXRP`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseXRP'])));
			...
			$req = $bdd->prepare("UPDATE `XRP` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseXRP` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseXRP'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseXRP']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `XRP` SET `ActionsRestantes`= 0 WHERE `AdresseXRP` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseXRP'])));
			...
			$req = $bdd->prepare("UPDATE `XRP` SET `Gains` = `Gains`+ ? WHERE `AdresseXRP` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseXRP'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseXRP']) && empty($_GET['ajoutXRP']) && empty($_POST['adresseXRP']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `XRP` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseXRP` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseXRP'])));
			header("Location: faucet.php");
			exit();
		}
