...
*Le captcha est déjà vérifié; On a $_SESSION['adresseZEC']
*
*Si on vient de l'accueil :
*On a $_POST['adresseZEC']
*
*Si on vient de faucet :
*on a $_SESSION['adresseZEC'] et $_GET['ajoutZEC']
*
*/
include 'Fonctions/connectionBDD.php';
$req = $bdd->query("SELECT `Gain2Min` FROM `CoursCryptos` WHERE `Code` = 'ZEC'");
...
if (!empty($_POST['adresseZEC'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseZEC'] = htmlspecialchars($_POST['adresseZEC']);
		echo "session : " . $_SESSION['adresseZEC'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `ZEC` WHERE `AdresseZEC` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseZEC'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `ZEC` (`AdresseZEC`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseZEC'])));
			...
			$req = $bdd->prepare("UPDATE `ZEC` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseZEC` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseZEC'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseZEC']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `ZEC` SET `ActionsRestantes`= 0 WHERE `AdresseZEC` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseZEC'])));
			...
			$req = $bdd->prepare("UPDATE `ZEC` SET `Gains` = `Gains`+ ? WHERE `AdresseZEC` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseZEC'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseZEC']) && empty($_GET['ajoutZEC']) && empty($_POST['adresseZEC']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `ZEC` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseZEC` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseZEC'])));
			header("Location: faucet.php");
			exit();
		}
