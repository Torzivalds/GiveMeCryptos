...
*Le captcha est déjà vérifié; On a $_SESSION['adresseDOGE']
*
*Si on vient de l'accueil :
*On a $_POST['adresseDOGE']
*
*Si on vient de faucet :
*on a $_SESSION['adresseDOGE'] et $_GET['ajoutDOGE']
*
*/
...
if (!empty($_POST['adresseDOGE'])) {
	...
	if ($response && $response->success) {
		...
		$_SESSION['adresseDOGE'] = htmlspecialchars($_POST['adresseDOGE']);
		echo "session : " . $_SESSION['adresseDOGE'];
		//On regarde si c'est un nouveau compte ou un compte existant
		$req = $bdd->prepare("SELECT `ID` FROM `DOGE` WHERE `AdresseDOGE` = ?");
		$req->execute(array(htmlspecialchars($_POST['adresseDOGE'])));
		$nbResultats = 0;
		while ($donnees = $req->fetch())
		{
			$nbResultats = 1;
		...
		if ($nbResultats == 0) {
			...
			$req = $bdd->prepare("INSERT INTO `DOGE` (`AdresseDOGE`, `Gains`, `DerniereAction`, `ActionsRestantes`, `DateCreation`) VALUES ( ?, 0, UNIX_TIMESTAMP(), 1, CURDATE() )");
			$req->execute(array(htmlspecialchars($_POST['adresseDOGE'])));
			...
			$req = $bdd->prepare("UPDATE `DOGE` SET `DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseDOGE` = ?");
			$req->execute(array(htmlspecialchars($_POST['adresseDOGE'])));
			header("Location: faucet.php");
			exit();
		}
...
elseif (!empty($_SESSION['adresseDOGE']) && !empty($_GET['ajout'])) {
	//On vient de faucet
	...
	while ($donnees = $req->fetch()) {
		...
		if ($balanceUser < 120) {
			...
			$req = $bdd->prepare("UPDATE `DOGE` SET `ActionsRestantes`= 0 WHERE `AdresseDOGE` = ?");
			$req->execute(array(htmlspecialchars($_SESSION['adresseDOGE'])));
			...
			$req = $bdd->prepare("UPDATE `DOGE` SET `Gains` = `Gains`+ ? WHERE `AdresseDOGE` = ?");
			$req->execute(array($gainTotal, htmlspecialchars($_SESSION['adresseDOGE'])));
			header("Location: faucet.php");
			echo "C'est ici ?";
			exit();
		}
...
elseif (!empty($_SESSION['adresseDOGE']) && empty($_GET['ajoutDOGE']) && empty($_POST['adresseDOGE']) && !empty($_POST['coinhive-captcha-token'])) {
	//On vient de captcha
	//On vérifie le captcha
	...
	if ($response && $response->success) {
		...
		while ($donnees = $req->fetch()) {
			...
			$req = $bdd->prepare("UPDATE `DOGE` SET `Gains` = `Gains`+ ?,`DerniereAction`= UNIX_TIMESTAMP() WHERE `AdresseDOGE` = ?");
			$req->execute(array( $gain2min, htmlspecialchars($_SESSION['adresseDOGE'])));
			header("Location: faucet.php");
			exit();
		}
